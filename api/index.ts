...
// This file is both the local server and the Vercel serverless function, so it
// lives under api/ where Vercel looks for functions — importing it from another
// directory would need an extension-qualified specifier to survive Node's ESM
// resolver, so the app is defined here rather than re-exported.
//
// On Vercel the frontend is served from the CDN and this module is imported and
// called per request, so it must not serve static files or bind a port.
